...
SweepStakesNFTsContract.Transfer.handlerWithLoader({
  ...
  handler: async ({ event, context }) => {
    ...
    const userEntity = await context.User.get(newOwner);
...
SweepStakesNFTsContract.Enter.handlerWithLoader({
  loader: async ({ event, context }) => {
  ...
  handler: async ({ event, context }) => {
    ...
    const contractName = "SweepStakes";
    const contractTotals = await context.ContractTotals.get(contractName);
    const userAddress = tokenEntity.userAddress;
    const userEntity = await context.Token.getUser(context.Token.token);
...
SweepStakesNFTsContract.Unstake.handlerWithLoader({
  loader: async ({ event, context }) => {
    const tokenId = event.params._tokenId.toString();
    context.Token.tokenLoad(tokenId, { loadUser: {} });
    const contractName = "SweepStakes";
...
SweepStakesNFTsContract.WinnerAssigned.handlerWithLoader({
  loader: async ({ event, context }) => {
